  ...
  <div class="page" v-if="controller">
    ...
    <div class="card" style="padding: 0; overflow-x: auto" v-if="quality">
      ...
      <p class="hint" style="padding: 0 0.6rem 0.6rem">
        Расчёт «вне диапазона» — по <b>текущему</b> профилю порогов (смена порогов
        меняет и прошлое в этом представлении; печатные формы считаются по
        профилю, действовавшему на момент измерения). MKT — среднекинетическая
        температура: экспоненциально взвешивает тепловое воздействие, короткий
        перегрев влияет сильнее долгого лёгкого. Отсутствие данных
