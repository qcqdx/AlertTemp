...
class Settings(BaseSettings):
    ...
    # окно группировки событий: одновременные открытия/закрытия (групповой
    # сбой) схлопываются в одно сообщение-сводку; 0 = слать поштучно сразу.
    # Цена — задержка одиночного оповещения на это окно.
    notify_group_window_s: float = 30.0
    # эскалация: не подтверждён через delay -> круг 2, через 2*delay -> круг 3;
    # 0 = эскалация выключена (все получатели равны кругу 1)
    notify_escalation_delay_s: int = 900
    # ack-кнопки в сообщениях Telegram + поллер подтверждений
    notify_telegram_ack_buttons: bool = True
    # ежедневный дайджест «система жива», час локального пояса; -1 = выключен
    notify_heartbeat_hour: int = 9
