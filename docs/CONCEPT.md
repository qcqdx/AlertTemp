...
- **Фактический профиль нагрузки** (замерено на боевом контроллере
  ESP8266 + 3×DS18B20): пачка из 3 PUBLISH примерно каждую секунду,
  ~270 тыс. измерений/сутки с одного контроллера; прошивка необслуживаемая,
  темп изменить нельзя. Экстраполяция на 30 холодильников — ~8 млн строк
  в сутки, поэтому агрегаты, сжатие и ретеншн — обязательная часть системы.
- **Схема измерений**: `measurement(time timestamptz, sensor_id, value real)`
  — hypertable + continuous aggregate `measurement_1m`
  (sum/min/max/count по минутам, обновление раз в минуту); все bucket-запросы
  графиков идут в агрегат и не сканируют сырые данные.
- **Ретеншн** (миграция 0003, меняется SQL-командами
  `remove_/add_retention_policy`): сырые данные — 365 дней, сжатие чанков
  старше 7 дней (segment by sensor); минутный агрегат — 3 года (нормативный
  горизонт). Дисковая оценка при 30 холодильниках: сырой год со сжатием —
  единицы–десятки ГБ, агрегаты — незначительны; конфигурация ВМ 100 ГБ
