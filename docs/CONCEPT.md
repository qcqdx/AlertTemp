...
- Контроллеры публикуют показания в **MQTT-брокер** (Mosquitto, `10.10.0.3:1883`).
- **Один контроллер = 3 датчика температуры.** Это физическая единица
  оборудования; в интерфейсе датчики группируются по три.
- Payload сообщения — **число (float) в виде текста**, температура в °C.
- Топик идентифицирует датчик (например, `fridge01/sensor2`). Структура топиков
  заранее неизвестна и не контролируется нами.
- Контроллеры **не буферизуют** данные и не имеют обратного канала: если
  сообщение не принято — оно потеряно. Отсюда требование к постоянной
  доступности приёмника и к детекции «молчащих» датчиков.
...
```
Controller (контроллер)          Sensor (датчик)
├─ id                            ├─ id
├─ name («Холодильник аптеки»)   ├─ controller_id (≤3 на контроллер)
├─ location (помещение/этаж)     ├─ mqtt_topic  ← уникальный, из железа
├─ status: active|archived       ├─ name («Верхняя полка»)
└─ notes                         ├─ position: 1..3
...
**Обнаружение устройств.** Ingest слушает широкий префикс. Топики, не
привязанные к датчикам, копятся в таблице `discovered_topics` (топик, первое
и последнее сообщение, частота, последнее значение). В UI — раздел
**«Обнаруженные устройства»**: администратор видит живой список «ничьих»
топиков и в два клика привязывает их к новому или существующему контроллеру.
Это заменяет и `subscribe("#")` со стихийным созданием таблиц, и ручную
настройку вкладок из исходного проекта.
